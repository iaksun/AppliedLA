...
# ╔═╡ 1bbcd0b3-75db-49ad-85fa-8a5a9af1a613
using PlotlyJS
...
* `` \color{red} \Large A \;Few \;Applications \;(Cont'd)``
  ...
  + ``\color{red} Compression \;\;(Image, \;Data, \;etc.)``
...
img = rand(1,5) #Arrays of "plain numbers" are not displayed graphically, because they might represent something numerical rather than an image. 
...
RGB{Float32}(0,0,255)/255 # have to normalize it by 255 as RGB assumes numbers between 0 and 1. However, if they are larger than 1, the respected color means to be saturated
...
md"""
##### Calculating the size of an image in memory

First thing you need to know is the heigth and width in terms of number of pixels.

Let us look at the following two images:
* one on the left is on a $8 \times 8$ PPi scale,
* one on the right is on a $16 \times 16$ PPi scale.

Notice that both images are the same physical size however the 16 x 16 image is better quality because more pixels mean better resolution.

Assume that the color depth is 24 bits.
"""
...
# ╔═╡ d9777d18-e03d-4a58-8701-ca70bffa2b09
typeof(imgc1); # gives the type of 'imgc1' as a matrix of RGB data

# ╔═╡ fb11137a-2118-4524-827c-c44b5073ba8d
size(imgc1) # size shows that it is 8x8 matrix and its entries are RGB{Float64}

# ╔═╡ 757ca38f-5070-4c77-b792-d162b3254a12
md"
One of the nice things about this representation of the image is that all of the indices in imgc1[i,j,...] correspond to locations in the image: you don't need to worry about some dimensions of the array corresponding to ''color channel'', and you're guaranteed to get the entire pixel contents when you access that location.

However, sometimes to work with other packages, you'll need to convert a $m \times n$ RGB image data to $m \times n \times 3$ numeric array with separate channels for the color components and vice versa. The functions `channelview` and `colorview` are designed for this purpose.
"
...
md"
Maybe we want the color channel be the last dimension, rather than the first. We can achieve that using permutedims:
"

# ╔═╡ 0947c623-7ea2-4ecc-a971-adbf5982fd0d
img_HWC = permutedims(img_CHW, (2, 3, 1)); # 8 x 8 x 3 1st H, 2nd W, 3rd Color

# ╔═╡ a31d11b0-c7b2-48fd-9fac-eeb47d85cf5c
md"
A few points are in order to emphasize: 

*  Notice that in HWC ordering, the first 8x8 matrix is for `Red Channel`, the second is for `Green Channel` and the third is for `Blue Channel`.
*  Based on the picture given below, it is mostly white and that is the reason why most of the pixels in all channels are $\rm 1.0$'s. Remember that $\rm RGB(1.0,1.0,1.0)$ corresponds to the white color.
*  The pixels that are red, for example $[2,2,{\color{red} 1}]$ and $[2,3,{\color{red} 1}]$, are $1.0$'s in only the red channel and $0$'s in other two channels, $[2,2:3,{\color{green} 2}]$ and $[2,2:3,{\color{blue} 3}]$. 
"
...
img_rgb = colorview(RGB, img_CHW); # 8 x 8
...
begin
p_image = plot(imgc1, xticks = 1:8, yticks = 1:8, xlim = (1,8), ylim = (1,8), aspect_ratio = 1);
p_image2 = plot(imgc2, xticks = 0:2:16, yticks = 0:2:16, xlim = (1, 16), ylim = (1,16), aspect_ratio = 1); # not possible to introduce grid lines when it is image
plot(p_image, p_image2, layout= (1,2))
end
...
md" 
However, **for a realistic image**, we have more pixels in both direction: 
suppose 600 pixels for height and 800 pixels for width are used. Hence

>600 x 800 x 24 = 11,520,000 Bits OR 11.52 Mbs (Megabits) OR 11.52/8=1.44 MB (MegaBytes)
> **Size on Disk = Width x Height x Color Depth (bytes)**

**For a typical video**, 1280 (W) x 720 (H) x 3 (C) = 2.76 MB for a single frame

As it comes with a quick succession of frames, 30 frames/second, then the size becomes

1280 (W) x 720 (H) x 3 (C) x 30 second = 82.86 MB/s → x 60 seconds = 4.98GB/min

This is a huge data to transfer and/or process for confort. So one needs to compress this information with no loss or with an acceptable level of loss. 

$\bf This \;is \;what \;we \;refer \;to \;as \;Compression$

There are many ways to compress an image data, but the one which will be introduced here is a Linear Algebra Technique, known as $\bf Singular \;Value \;Decomposition \;(SVD)$, and regardless of the field, it has become a key tool in data centeric applications.
"
...
butterfly = load("/Users/irsadiaksun/Dropbox/AppliedLA/LectureNotes/Lecture_2_Matrices/milkweed-rainbow-butterfly.jpeg") # loaded from the directory in my computer
...
md"""
##### ''Locations in an image: Indexing$^{[2]}$
...
[^2] This `md` portion is the copy of the notebook by Prof. Alan Edelman, David P. Sanders & Charles E. Leiserson preperared for the course [``Computational \;Thinking``] (https://computationalthinking.mit.edu/Spring21/images/) at MIT.
"""
...
imresize(butterfly[1:5:1164, 1:5:1552], (1164,1552))

# ╔═╡ d5d2a909-a172-4352-aafd-08d3c1cf56f4
md"#
### Brief Intro to factorization and SVD

[**_Factorization_**] (https://www.youtube.com/watch?v=or6C4yBk_SY) is one of the fundamental topics in Matrix representations in Linear Algebra and there are five fundamental and popular factorizations of matrices, which are given below:

*  $\bf A = LU$ Factorization - Elimination for solving Linear System of Equations (LSE)
*  $\bf A = QR$ Factorization - Least Square solutions, e-value and e-vector computations
*  $\bf S = Q \Lambda Q^T$ Factorization - For symmetric matrices ($\bf S$), diagonalization
*  $\bf A = X \Lambda X^{-1}$ Factorization - For square and full e-vector matrices, diagonalization
*  $\color{red} \bf A = U \Sigma V^T$ Factorization - Singular Value Decomposition (SVD)


Before the introduction of these factorizations briefly, let us see the application of SVD on Image Compression.

##### Application of SVD for image compression

"

# ╔═╡ 68ddd494-18b4-4b1b-9c14-eb8edfa99867
md"
...
For the sake of simplicity, let us work with the gray scale image of the butterfly:
"

# ╔═╡ a7a46218-6ca9-4041-a9f1-7edc6ef5e458
gray_butterfly = Gray.(butterfly) # By elliminating the color, now it is 2D array
...
nofpixel_skip_h, nofpixel_skip_w = 5, 5;
...
# ╔═╡ 8d202f97-e3f8-4872-af70-f6431cb8de70
size(butterfly_sliced)
...
# ╔═╡ 87647bb0-3df4-438f-b540-a10ed09cc986
size(U[:,1:50])

# ╔═╡ e353c332-d834-4f5d-861e-ff3a683e20c2
size(Diagonal(σ[1:50]))

# ╔═╡ 1dd4c51f-a296-4c04-b9f8-5aef6fe67a5f
size(V[:,1:50]')
...
# ╔═╡ ae823d1c-d965-4b28-a9a0-160f101ee5e3
mosaicview(imresize(butterfly_sliced, (234, 312)), imresize(GB_rank50, (234, 312)), nrow = 1)

# ╔═╡ 5c292b05-5d5d-43ea-8ac2-9c6578d14d86
mosaicview(imresize(butterfly_sliced, (1164,1552)), GB_rank20, nrow = 1)

# ╔═╡ 7d0f3c58-006f-48bf-be06-d2528ca8d697
zz = [2*x*y for x in -10:10, y in -10:10]
...
# ╔═╡ e9d3f12a-0a8f-41c4-a0d9-09406fbc7e03
# begin
# n = 50
# x = rand(n)
# y = rand(n)
...
Uz,σz,Vz = svd(zz, full = true)

# ╔═╡ e6f2debc-122c-4955-96bc-1261e9a5a964
zz_rank1 = σz[1] * Uz[:,1] * Vz[:,1]'
